...
@jax.jit
def inverse_leaky_relu(y: chex.Array):
...
@jax.jit
def inverse_softplus(y: chex.Array):
    """Inverse of jax.nn.softplus, adapted from TensorFlow Probability."""
    threshold = jnp.log(jnp.finfo(jnp.float32).eps) + 2.0
    is_too_small = y < jnp.exp(threshold)
    is_too_large = y > -threshold
    too_small_value = jnp.log(y)
    too_large_value = y
    y = jnp.where(is_too_small | is_too_large, 1.0, y)
    x = y + jnp.log(-jnp.expm1(-y))
    return jnp.where(
        is_too_small, too_small_value, jnp.where(is_too_large, too_large_value, x)
    )
...
def create_activation_bijector(activation: str) -> distrax.Bijector:
...
def cartesian_to_polar(x: chex.Array, y: chex.Array) -> Tuple[chex.Array, chex.Array]:
...
class NormalizingFlow(abc.ABC, nn.Module):
    ...
    @abc.abstractmethod
    def inverse(self, inputs: chex.Array) -> chex.Array:
...
class MaskedCouplingFlowConditioner(nn.Module):
    ...
    def __call__(self, inputs: chex.Array) -> chex.Array:
        ...
        inputs = nn.Dense(np.prod(self.event_shape) * self.num_bijector_params)(inputs)
        inputs = jnp.reshape(
            inputs,
...
class OneDimensionalNormalizingFlow(NormalizingFlow):
    ...
    def setup(self):
        ...
        for index in range(self.num_layers):
            scale = self.param(f"scale_{index}", nn.initializers.lecun_normal(), (1, 1))
            shift = self.param(f"shift_{index}", nn.initializers.lecun_normal(), (1, 1))
...
class PointwiseNormalizingFlow(NormalizingFlow):
    ...
    def inverse(self, inputs: chex.Array) -> chex.Array:
        ...
        return jnp.concatenate((first_coords_inverted, second_coords_inverted), axis=-1)
...
class GradientBasedConditioner(nn.Module):
    ...
    def __call__(self, inputs: chex.Array) -> chex.Array:
        num_dims = inputs.shape[-1]
        w = self.param(
            "w", nn.initializers.normal(0.01), (num_dims, self.projection_dims)
        )
...
class ShearNormalizingFlow(NormalizingFlow):
    ...
    def forward(self, inputs: chex.Array) -> chex.Array:
        ...
        if self.switch:
            first_coords_transformed, second_coords = (
                second_coords,
                first_coords_transformed,
            )
...
class SymplecticLinearFlow(NormalizingFlow):
    ...
    def setup(self):
        ...
        self.rotate_val = self.param("rotate_val", nn.initializers.zeros, (num_dims,))

    def extract_coords(self, inputs: chex.Array) -> chex.Array:
    ...
    def shift(
        self, first_coords: chex.Array, second_coords: chex.Array, forward: bool
    ) -> Tuple[chex.Array, chex.Array]:
    ...
    def rotate(
        ...
        if not forward:
            theta = -theta
        first_coords_rotated = first_coords * jnp.cos(theta) - second_coords * jnp.sin(
            theta
        )
        second_coords_rotated = first_coords * jnp.sin(theta) + second_coords * jnp.cos(
            theta
        )
...
def create_masked_coupling_flow(
    ...
    def bijector_fn(params):
        return distrax.RationalQuadraticSpline(
            params, range_min=config.spline_range_min, range_max=config.spline_range_max
        )
    ...
    for _ in range(config.num_flow_layers):
        conditioner = MaskedCouplingFlowConditioner(
    ...
    return MaskedCouplingNormalizingFlow(
        event_shape=init_shape[-1], bijector_fn=bijector_fn, conditioners=conditioners
    )


def create_pointwise_flow(
    config: ml_collections.ConfigDict, init_shape: Tuple[int, int]
) -> PointwiseNormalizingFlow:
    """Builds the pointwise normalizing flow model."""
    activation = create_activation_bijector(config.activation)
    base_flow_input_dims = init_shape[-1] // 2
    base_flow = OneDimensionalNormalizingFlow(
        num_layers=config.num_flow_layers, activation=activation
    )
    return PointwiseNormalizingFlow(
        base_flow=base_flow, base_flow_input_dims=base_flow_input_dims
    )


def create_shear_flow(
    config: ml_collections.ConfigDict, init_shape: Tuple[int, int]
...
class CoordinateEncoder(abc.ABC, nn.Module):
    ...
    @abc.abstractmethod
    def __call__(
        self, positions: chex.Array, momentums: chex.Array
    ) -> Tuple[chex.Array, chex.Array]:
...
class CoordinateDecoder(abc.ABC, nn.Module):
    ...
    @abc.abstractmethod
    def __call__(self, actions, angles):
...
class MLPDecoder(CoordinateDecoder):
    ...
    def __call__(
        self, latent_positions: chex.Array, latent_momentums: chex.Array
    ) -> Tuple[chex.Array, chex.Array]:
        # Encode input coordinates.
...
class ActionAngleNetwork(nn.Module):
    ...
    def predict_single_step(
        self, positions: chex.Array, momentums: chex.Array, time_deltas: chex.Array
    ...
    def predict_multi_step(
        self,
        init_positions: chex.Array,
        init_momentums: chex.Array,
        ...
        if self.polar_action_angles:
            future_angles = (future_angles + jnp.pi) % (2 * jnp.pi) - (jnp.pi)
            future_latent_positions, future_latent_momentums = jax.vmap(
                polar_to_cartesian, in_axes=(None, 0)
            )(actions[0], future_angles)
        else:
            future_latent_positions, future_latent_momentums = jax.vmap(
                lambda x, y: (x, y), in_axes=(None, 0)
            )(actions[0], future_angles)
...
class EulerUpdateNetwork(nn.Module):
    ...
    @nn.compact
    def __call__(
        self, positions: chex.Array, momentums: chex.Array, time_delta: chex.Array
